...
Side Panel → Content Script 通过 `chrome.tabs.sendMessage` 直接发送，格式为 `{ tool: '工具名', args?: {...} }`，支持的 tool 包括 `get_domain`、`get_page_structure`、`grep`、`inject_css`、`rollback_css`、`replace_css`、`get_active_css`。Content Script 的响应作为 `sendMessage` 的 response 回调直接返回，无需经过 Service Worker。
...
工具按执行位置分为两类：需要访问 live DOM 的工具（`get_page_structure`、`grep`、`apply_styles`）通过 Content Script 执行；在 Side Panel 中执行的工具包括 `edit_css`（精准 CSS 文本替换后通过 `replace_css` 消息重新注入）、`get/update_user_profile`、`load_skill` 等读写 chrome.storage 的操作。
...
工具名 `apply_styles`，支持两种 mode：`save`（添加全新规则时使用，注入 CSS 到页面，保存到当前会话，并同步当前域名的活动样式镜像，供刷新后自动恢复）、`rollback_all`（回滚当前会话的全部样式）。参数包括 `css`（save 模式必填）和 `mode`（必填）。修改已有样式请使用 `edit_css` 工具（精准替换，省 token）。
...
- `rollback_all`：向 Content Script 发送回滚全部指令，清除当前会话的 `stylesKey`，再同步清空 `active_styles:{domain}`，最后更新样式摘要。返回 `'已回滚所有样式。当前无已应用样式。'`。
- `save`：向 Content Script 发送注入指令；若 Content Script 返回 `scripting-api` 降级信号，则由 Side Panel 调用 `chrome.scripting.insertCSS` 完成注入。之后使用 `mergeCSS` 将新 CSS 与当前会话已有 CSS 去重合并，写回 `stylesKey`，再镜像到 `active_styles:{domain}`，并更新样式摘要。返回 `'样式已应用。当前完整样式：\n{merged}'`（便于模型确认最终状态）。
...
精准编辑已应用的 CSS 样式，通过文本精确替换修改已存储的 CSS 并重新注入页面。适用于修改已有样式，比 `apply_styles(save)` 更精准、省 token。

#### 3.10.1 Tool 定义

**文件位置：** `extension/sidepanel/tools.js`

工具名 `edit_css`。参数包括 `old_css`（必填，要替换的 CSS 片段，必须与 `[当前已应用样式]` 中的内容精确匹配，包含空格和换行）和 `new_css`（必填，替换后的内容，空字符串表示删除该片段）。每次只替换第一处匹配。

典型用途：修改某个属性值（如改颜色、调字号）、删除某条规则、替换某个选择器的整个规则块。

#### 3.10.2 实现

**文件位置：** `extension/sidepanel/tools.js`（Side Panel 端）、`extension/content/content.js`（Content Script 端）

**Side Panel 端 `runEditCSS(oldCSS, newCSS)`：**
1. 从 `currentSession.stylesKey` 读取当前存储的完整 CSS。
2. 检查 `stored.includes(oldCSS)`——若未找到匹配，返回失败提示和当前完整样式，供模型修正 `old_css`。
3. 用 `stored.replace(oldCSS, newCSS)` 执行替换，得到 `updated`。
4. 若 `updated.trim()` 非空，写回 `stylesKey`；否则删除该 key（全部样式已清空）。
5. 向 Content Script 发送 `replace_css` 消息，将更新后的完整 CSS 一次性替换到页面。
6. 同步 `active_styles:{domain}` 镜像，更新样式摘要。
7. 返回 `'样式已更新。当前完整样式：\n{trimmed}'` 或 `'样式已全部删除。当前无已应用样式。'`。

**Content Script 端 `replace_css` 消息处理器：**
清空 `cssStack`，若传入的 `args.css` 非空则将其压入栈，再调用 `applyCurrentCSS()` 重新注入。此操作替换整个样式栈，确保页面样式与存储完全同步。
...
`buildSessionContext(domain, sessionMeta, profileHint, currentCSS)` 构建会话上下文文本，包含域名、会话标题、**当前已应用的完整 CSS 代码**（以 `[当前已应用样式]` 标记，无样式时注明"无"）和用户风格偏好一句话提示（如有，注明可通过 `get_user_profile` 获取详情）。

完整 CSS 直接注入 system prompt 的作用：模型在使用 `edit_css` 时可直接从上下文中复制 `old_css`，无需额外工具调用查询现有样式，减少一次往返。`agentLoop` 在构建 system prompt 前从 `currentSession.stylesKey` 读取当前 CSS 并传入此函数。

`getProfileOneLiner()` 从 chrome.storage 读取用户画像，截取第一行前 100 字符作为提示。
...
**样式应用确认浮层：** Agent 本轮完成后，若执行了 `apply_styles(save)` 或 `edit_css`，在输入框上方浮现确认/撤销按钮。等整轮结束后统一出一次浮层。显示 `[✓ 确认效果] [↶ 全部撤销]`。浮层消失条件：点击确认/撤销、用户发新消息（隐式确认）、60s 超时。
...
`executeTool(name, args)` 按工具名分派执行。需要 Content Script 的工具（`get_page_structure`、`grep`、`apply_styles`）通过 `sendToContentScript` 发送消息；Side Panel 本地工具（`edit_css`、`get_user_profile`、`update_user_profile`、`load_skill`、`save_style_skill`、`list_style_skills`、`delete_style_skill`、`TodoWrite`、`Task`）直接调用对应函数。其中 `edit_css` 在 Side Panel 完成文本替换后通过 `sendToContentScript` 发送 `replace_css` 消息重新注入页面。
...
Content Script 端通过 `chrome.runtime.onMessage.addListener` 注册消息监听器，根据 `message.tool` 分派执行：`get_domain` 返回 `location.hostname`；`get_page_structure` 调用 `getPageStructure()`；`grep` 调用 `runGrep()`；`inject_css` 调用 `injectCSSAuto()`（CSP 兼容分派）；`rollback_css` 调用 `rollbackCSS()`；`get_active_css` 返回当前 cssStack 内容；`replace_css` 清空 cssStack、压入新 CSS（若非空）、调用 `applyCurrentCSS()` 重新注入（供 `edit_css` 工具使用）。监听器返回 `true` 以支持异步 `sendResponse`。
...
**System Prompt（`SYSTEM_BASE`）：** 定义 StyleSwift 身份为网页样式个性化智能体，说明工作方式（使用工具完成任务、优先行动而非解释、完成后简要总结），列出所有可用工具名称，包含 CSS 生成指引（使用具体选择器、加 !important、用 hex/rgba 颜色）、**样式编辑策略**（修改已有样式时优先用 `edit_css` 精准替换；添加全新规则时用 `apply_styles(mode:save)`；`edit_css` 的 `old_css` 必须从 `[当前已应用样式]` 中精确复制；全部重来用 `apply_styles(mode:rollback_all)`）和风格技能使用规范。

**工具集合：** `BASE_TOOLS` 包含 10 个基础工具（含新增的 `edit_css`）+ TodoWrite；`ALL_TOOLS` 在此基础上加 Task 工具。
...
4. **构建 system prompt** — `SYSTEM_BASE` + `buildSessionContext`（域名/会话/当前完整 CSS/画像提示）；`agentLoop` 在此步从 `currentSession.stylesKey` 读取当前 CSS 并传入。
...
当 Agent 本轮结束且执行了 `apply_styles(mode='save')` 或 `edit_css` 时，在输入框上方浮现确认/撤销按钮。
...
| 点击"全部撤销" | Side Panel 直接调用 `executeTool('apply_styles', { mode: 'rollback_all' })` 回滚全部样式，浮层消失 |
